...
contract LiquidityMigrator {
    IUniswapV2Router02 public router; //uniswap
    IUniswapV2Pair public pair;//uniswap
    IUniswapV2Router02 public routerFork; //our router fork
    IUniswapV2Pair public pairFork;//our pair fork
    BonusToken public bonusToken; // reward token
    ...
    function migrate()external{
        ...
        router.removeLiquidity(
            ...
            address(this),
            block.timestamp);
        uint  token0Balance = token0.balanceOf(address(0));
        uint token1Balance = token0.balanceOf(address(0));
